...
export class PageInfo<T extends Node = any> {
  ...
  constructor(edges: T[], total: number, pagination: Partial<Pagination> = {}) {
    if (edges.length > 0) {
      if (pagination.first) { // forward pagination
        this.hasNextPage = total > edges.length;
        this.endCursor = Connection.btoa(edges[edges.length - 1].id);
      } else if (pagination.last) { // backward pagination
...
export class Connection<T extends Node = any> {
  ...
  constructor(edges: T[], total: number, pagination: Pagination) {
    this.total = total;
    this.edges = edges.map((node) => new Edge<T>(node));
    this.pageInfo = new PageInfo<T>(edges, total, pagination);
  }
}
